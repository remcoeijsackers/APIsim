...
class customrequest(requests):
    def __init__(self, verbose=False, fallback_enabled=True, repeat=0) -> None:
        super().__init__()
        self._response = None
        self._units = []
        ...
        with TorRequests() as tor_requests:
            with tor_requests.get_session(retries=3) as sess:
                def process(url):
                    ...
                    try:
                        if req_unit.token != "":
                            headers = {'Authorization': 'access_token %s'.format(req_unit.token)}
                        else: 
                            headers = None
    ...
    def multi_request(self, req_unit: request_unit):
        ...
        def req(req_unit):
            ...
            if res.status_code != 200:
                ...
                status = "Succes"
            self._response = response_unit(req_unit.url[0],res.content, req_unit.mode, res.elapsed.total_seconds(), res.status_code, status)
            print(self._response)
            self._calls += 1
            if self.print_steps:
                print(str(self._calls) + " '" + req_unit.mode + "'" + ' on endpoint ' + req_unit.url[0])
            self._units.append(self._response)
